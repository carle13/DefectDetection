...
def computeQ(input_file):
    ...
    sys.find_neighbors(method='cutoff',cutoff=coordCutoff)
    ...
    sysMg.atoms=Mg
    sysMg.find_neighbors(method='cutoff',cutoff=coordCutoff)
    ...
    sysSi.atoms=Si
    sysSi.find_neighbors(method='cutoff',cutoff=coordCutoff)
    ...
    sysOx.atoms=Ox
    sysOx.find_neighbors(method='cutoff',cutoff=coordCutoff)
...
#Iterate through all lammps dump files in input directory
list_file = sorted(glob.glob(inDir+'InputExtendedDefects/**/*.trj', recursive=True))
for input_file in list_file:
    computeQ(input_file)